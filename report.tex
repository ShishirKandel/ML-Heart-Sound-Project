...
\textit{Why a Brazilian dataset for a Nepal-motivated project?} Across standard public repositories (PhysioNet, Kaggle, UCI, Mendeley, Zenodo, IEEE DataPort) and the paediatric-cardiology literature, no publicly available Nepali PCG dataset turned up. We therefore use the PhysioNet 2022 CirCor DigiScope corpus \citep{challenge2022, circor_data} (rural Para\'{i}ba, Brazil) as a methodology-development proxy: it shares the Nepal context's paediatric population with high RHD-like lesion prevalence, rural deployment with generalist-clinician auscultation, Littmann 3200 hardware, and the ``auscultation $\rightarrow$ echocardiography referral'' workflow. Nepal deployment would require local validation (Section~\ref{sec:discussion}).

CirCor was built for this paediatric screening scenario: 942 patients in the public split. Up to 80\% of paediatric murmurs are \textit{innocent} (vibratory, benign), yet still need separating from pathological ones.
...
On MFCC features, Fuadah et al.\ \citep{fuadah2023} tested K-NN, Random Forest, ANN, and SVM, scoring K-NN 76.31\%, RF 68.76\%, ANN 65.18\%, SVM 58.81\%. With ten multiscale wavelet+fractal features instead, Vimalajeewa et al.\ \citep{wavelet2025} compared LR, K-NN, SVM, and a feed-forward NN; their SVM-RBF reached 76.61\% accuracy (AUC 0.741), the best published \textit{pure classical} result and our direct benchmark. Feature choice drives model rank on CirCor: MFCC favours K-NN, wavelet favours SVM.
...
\begin{table}[!htbp]
\centering
\caption{CirCor 2022 public-training cohort. Across the Murmur $\times$ Outcome cross-tab, the auscultatory-pathology link is real but imperfect, motivating the held-out Unknown-class analysis.}
\label{tab:eda}
\small
\begin{tabular}{lr}
\toprule
...
Across a 150-recording sample audited post-preprocessing, durations stay roughly consistent between classes (median $\sim$23\,s, IQR 19--29\,s) with clipping $<$0.1\%. Post-bandpass, the SNR proxy is marginally higher for Present (median $-0.81$\,dB) than Absent ($-4.71$\,dB), so gross quality differences cannot drive the classifier results. Full histograms live in \texttt{eda\_outputs/}.
...
Six preprocessing stages run in sequence: resampling to 4000\,Hz, DC-offset removal, a 4th-order zero-phase Butterworth bandpass at 25--450\,Hz (cardiac-sound band), spike removal at $30\times$ MAD, Daubechies-4 wavelet soft-thresholding (universal threshold $\sigma\sqrt{2\log n}$, MAD-estimated $\sigma$), and peak normalisation to $[-1,1]$.

\subsection{Feature engineering}

All 48 features (five engineered domains plus two quality features) come from \texttt{librosa} \citep{librosa2015}:
...
Accuracy across the $5 \times 5$ grid (mean over 5 folds) appears in Table~\ref{tab:ablation} and Figure~\ref{fig:ablation}.
...
All metrics on the full 48-feature set appear in Table~\ref{tab:full} and Figure~\ref{fig:metrics}. GPC leads on accuracy (\textbf{84.03\%}, 95\% CI [83.4\%, 84.7\%]), AUC (0.752), and calibration (ECE 0.046, Brier 0.128). Even its CI lower bound sits 6.8 points past the published pure-classical SOTA of 76.61\% \citep{wavelet2025}, so the +7.4-point lift holds. As for K-NN's degenerate F1 (0.101): scikit-learn does not natively pass \texttt{class\_weight} to K-NN, an implementation gap rather than a feature-space failure. Table~\ref{tab:sota} positions our result against published CirCor 2022 work.
...
GPC achieves the lowest ECE and Brier in Figure~\ref{fig:calib}. On the 156 held-out Unknown recordings, its predictive entropy is significantly above Absent (Mann-Whitney $p = 0.010$) but not above Present ($p = 1.0$); Present is, after all, GPC's hardest class.
...
Per Table~\ref{tab:fi} and Figure~\ref{fig:fi}, the top ten covers all five feature families (MFCC$\times$3, spectral$\times$2, heart-cycle$\times$2, fractal$\times$2, quality$\times$1), and nine of ten carry non-zero Lasso coefficients, empirically backing the multi-domain hypothesis.
\FloatBarrier
...
\begin{table}[!htbp]
\centering
\caption{AUC by imbalance strategy. Class-weighted loss beats SMOTE outright on our best model (GPC) and tops 4 of 5 models overall.}
\label{tab:smote}
\small
\begin{tabular}{lrrr}
\toprule
...
On every GPC metric, class weights beat SMOTE (acc 0.840 vs.\ 0.767; AUC 0.752 vs.\ 0.695; F1 0.436 vs.\ 0.413). RandomOverSampler sits between the two, in line with the geometric reading in Section~\ref{sec:discussion}.
\FloatBarrier
...
\begin{itemize}[leftmargin=*, itemsep=0.05em]
    \item \textbf{O1 (multi-domain pipeline beats classical SOTA): MET.} Past Vimalajeewa et al.'s 76.61\% / 0.741, GPC reaches 84.03\% accuracy and 0.752 AUC; that's 7.4 and 1.1 points up. The ablation grid shows monotonic accuracy gains across feature sets and pinpoints fractal features as the critical addition for GPC. The top-10 importance ranking covers all five engineered families.
    \item \textbf{O2 (calibrated uncertainty via GPC): PARTIALLY MET.} Across all five models, GPC posts the lowest ECE (0.046) and Brier score (0.128), backing the calibration claim. On Unknown recordings, predictive entropy runs significantly above Absent ($p = 0.010$) yet \textit{not} above Present ($p = 1.0$); Present cases are GPC's hardest class. So the first half of O2 is fully met, the second half only partially.
\end{itemize}

% ============================================================
\section{Discussion}\label{sec:discussion}

\textbf{Headline finding: GPC + 48 multi-domain features sets a new pure-classical SOTA on CirCor 2022.} Our 84.0\% accuracy (AUC 0.752) clears the published 76.61\% benchmark by 7.4 points, putting it in the range of deep-learning work on this dataset, for example, Walker et al.'s DBRes hybrid at 77.1\% weighted accuracy \citep{walker2022dbres}. This demonstrates that careful classical feature engineering on this paediatric screening dataset still has substantial room above existing classical baselines.

\textbf{The fractal jump.} The most striking experimental finding is GPC's F1 jumping from 0.000 to 0.404 the moment fractal features come in. These three (fractal dimension, sample entropy, Hurst exponent) sit outside the standard audio feature set. What they contribute is geometric: with them, the RBF kernel finds a space in which the Present class becomes separable. Neither MFCC nor wavelet features alone deliver this structure. If replicated on external paediatric PCG datasets, this would suggest complexity features should become a default component of Bayesian-kernel audio classifiers.
...
\textbf{Clinical interpretation for Nepal RHD screening.} GPC's 97.7\% specificity suits high-confidence referral well. Its 30.6\% sensitivity, on the other hand, runs below what a first-stage gate needs. For that role, Lasso-LR (sensitivity 61.4\%, specificity 73.6\%) works better. A two-stage deployment (Lasso-LR gate + GPC overlay) falls naturally out of the experimental design (Figure~\ref{fig:deployment}).
...
All experiment artefacts are provided alongside this report: \nolinkurl{heart_murmur_pipeline.ipynb} (full reproducible Jupyter notebook), \nolinkurl{run_models.py} (5$\times$5 ablation grid), \nolinkurl{run_smote.py} (imbalance sensitivity analysis), \nolinkurl{uq_unknown.py} (Unknown-class entropy validation), \nolinkurl{error_analysis.py} (Section~\ref{sec:erroranalysis}), per-fold result CSVs, EDA outputs in \nolinkurl{eda_outputs/}, and trained \nolinkurl{.pkl} models with scaler in \nolinkurl{models/}. Reproduction steps are documented in the project \nolinkurl{README}.
